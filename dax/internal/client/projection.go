...
func (in *itemNode) toAttribute() types.AttributeValue {
	...
	if len(in.children) != 0 {
		...
		if keys[0].index < 0 {
			...
			for k, v := range m {
				attr[k.name] = v.toAttribute()
			}
			val = &types.AttributeValueMemberM{Value: attr}
		} else {
			...
			attr := make([]types.AttributeValue, len(keys))
			for i, k := range keys {
				attr[i] = m[k].toAttribute()
			}
			val = &types.AttributeValueMemberL{Value: attr}
		}
	}
	return val
}
