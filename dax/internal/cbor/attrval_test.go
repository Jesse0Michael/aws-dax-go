...
func TestAttrVal(t *testing.T) {
	cases := []struct {
		...
		{val: &types.AttributeValueMemberN{Value: "-314E-2"}},
		//{val: &types.AttributeValueMemberN{Value: stringptr("3.14")}},	// Decimal.String() return 314E-2
...
func TestDecodeIntBoundariesFromCbor(t *testing.T) {
	for _, e := range []IntBoundary{
		...
		if eAttr := (&types.AttributeValueMemberN{Value: e.value.String()}); !reflect.DeepEqual(eAttr, a) {
